...
<CodeRunner
  variant="guided"
  title="列表 + 详情两级抓取"
  prompt="任务:1)从 list_html 提取每个 a 的 href 和文本;2)根据 URL 从 detail_pages 取出对应详情 HTML;3)解析作者并合并到记录中。三步都要补全。"
...
<CodeRunner
  variant="challenge"
  title="用 set 给链接去重"
  prompt="任务:links 里有重复链接。用 set 维护已见集合 seen,只保留首次出现的链接到 unique 列表。补全去重逻辑。"
